...
import type { ContactMessage, PaginationMeta } from '../../types/api'
...
export function AdminContactMessagesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        setLoading(true)
        setError(null)
        const response = await api.getAdminContactMessages(token, {
  ...
  return (
    <section className="space-y-4">
      ...
      {!loading && items.length > 0 ? (
        <>
          <DataTable headers={['Name', 'Email', 'Message', 'Date']}>
            {items.map((message) => (
              <tr key={message.id}>
