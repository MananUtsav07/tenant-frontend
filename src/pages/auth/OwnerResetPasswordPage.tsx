...
export function OwnerResetPasswordPage() {
  ...
  usePageSeo({
    ...
    description: 'Set a new password for your Prophives owner account.',
  })
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const response = await api.ownerResetPassword({
        token,
        password,
      })
  ...
  return (
    <AuthSplitLayout
      ...
      title="Set a fresh password for your owner command center"
      description="Use the secure reset link from your email to update your owner password and return to portfolio operations with confidence."
      highlights={[
        { icon: <KeyRound className="h-4 w-4" />, text: 'Choose a new password with at least 8 characters' },
        { icon: <ShieldCheck className="h-4 w-4" />, text: 'Invalid or expired links are safely rejected' },
        { icon: <Sparkles className="h-4 w-4" />, text: 'Your organization workspace and access stay unchanged' },
      ]}
      panelTitle="Create new owner password"
    ...
    >
      ...
      <p className="mt-4 text-sm text-[var(--ph-text-muted)]">
        Need a new link?{' '}
        <Link className="ph-link font-semibold" to={ROUTES.ownerForgotPassword}>
